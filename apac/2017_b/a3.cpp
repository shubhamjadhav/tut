...
double f(int i,int j, vbb &is,vdd see,int steps,vbb &p){
	...
	if(i>0){
		...
		if(!is[i-1][j]){
			is[i-1][j]=1;
			ans = max(ans,tt + f(i-1,j,is,see2,steps-1,p));
			is[i-1][j]=0;
		}
		else{
			see2[i-1][j]*=(1.0-tt);
			ans = max(ans,see2[i-1][j] + f(i-1,j,is,see2,steps-1,p));
		}
	}
	if(i<n-1){
		if(p[i+1][j])
	...
	if(j>0){
		...
		if(!is[i][j-1]){
			is[i][j-1]=1;
			ans = max(ans,tt + f(i,j-1,is,see2,steps-1,p));
			is[i][j-1]=0;
		}
		else{
			see2[i][j-1]*=(1.0-tt);
			ans = max(ans,see2[i][j-1] + f(i,j-1,is,see2,steps-1,p));
		}
	}
	if(j<m-1){
		if(p[i][j+1])
